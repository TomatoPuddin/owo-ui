...
public class ConfigureHotReloadScreen extends BaseUIModelScreen<FlowLayout> implements CommandOpenedScreen {
    ...
    public ConfigureHotReloadScreen(Identifier modelId, @Nullable Screen parent) {
        super(FlowLayout.class, DataSource.asset(new Identifier("owoui", "configure_hot_reload")));
        this.parent = parent;
    ...
    protected void build(FlowLayout rootComponent) {
        rootComponent.childById(LabelComponent.class, "ui-model-label").text(Text.translatable("text.owoui.configure_hot_reload.model", this.modelId));
        ...
        rootComponent.childById(ButtonComponent.class, "choose-button").onPress(button -> {
            CompletableFuture.runAsync(() -> {
                var newPath = TinyFileDialogs.tinyfd_openFileDialog(Language.getInstance().get("text.owoui.configure_hot_reload.choose_ui_model_source"), FabricLoader.getInstance().getGameDir().toString(), null, null, false);
    ...
    private void updateFileNameLabel() {
        this.fileNameLabel.text(Text.translatable(
                "text.owoui.configure_hot_reload.reload_from",
                this.reloadLocation == null ? Text.translatable("text.owoui.configure_hot_reload.reload_from.unset") : this.reloadLocation
        ));
    }
}
